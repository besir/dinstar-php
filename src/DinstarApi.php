...
class DinstarApi
{
	...
	public function sendSms(
		...
		?bool $requestStatusReport = null): SendSmsResponse
	{
	...
	public function getPortInfo(array $infoType, ?array $ports = null): GetPortInfoResponse
	{
		...
		$reqResult = $this->_makeRequest('/api/get_port_info', 'GET', $params);
		$dinstarErrorCode = $reqResult['decodedBody']['error_code'] ?? null;
		$gatewaySn = $reqResult['decodedBody']['sn'] ?? null;
		$opSuccess = $reqResult['httpSuccessful'] && $dinstarErrorCode === 200;
		$collection = null;
		...
		if ($opSuccess && isset($reqResult['decodedBody']['info']) && is_array($reqResult['decodedBody']['info'])) {
			$items = array_map(fn($item) => new PortInfoItemData(port: $item['port'] ?? null, type: $item['type'] ?? null, imei: $item['imei'] ?? null,
				imsi: $item['imsi'] ?? null, iccid: $item['iccid'] ?? null, number: $item['number'] ?? null, reg: $item['reg'] ?? null,
				slot: $item['slot'] ?? null, callState: $item['callstate'] ?? null, signal: $item['signal'] ?? null, gprs: $item['gprs'] ?? null,
				remainCredit: $item['remain_credit'] ?? null, remainMonthlyCredit: $item['remain_monthly_credit'] ?? null,
				remainDailyCredit: $item['remain_daily_credit'] ?? null, remainDailyCallTime: $item['remain_daily_calltime'] ?? null,
				remainHourlyCallTime: $item['remain_hourly_calltime'] ?? null, remainDailyConnect: $item['remain_daily_connect'] ?? null,
		...
		return new GetPortInfoResponse($opSuccess, $reqResult['httpCode'], $dinstarErrorCode, $collection,
			$reqResult['httpSuccessful'] && $reqResult['decodedBody'] !== null ? null : $reqResult['rawBody'],
			$reqResult['curlError'] ?? (! $opSuccess && $dinstarErrorCode !== null ? "Dinstar API error: {$dinstarErrorCode}" : null), $gatewaySn);
	}
